			...
			{#each bahanItem as bahan, index}
				<div class="grid grid-cols-5 w-full h-15 pr-5 mt-2 border-b-2">
					...
					<button
						...
						class={padShow[index] && !padMode[index] ? 'bg-orange-100 text-sm' : 'bg-white text-sm'}
						>{bahan.belanjaCount} {bahan.satuanBeli}</button
					>
					<button
						on:click={() => {
							padShow[index] = true;
						...
						class={padShow[index] && padMode[index]
							? 'bg-orange-100 text-sm col-span-2'
							: 'col-span-2 bg-white text-sm'}>{rupiah(bahan.harga)}</button
					>
