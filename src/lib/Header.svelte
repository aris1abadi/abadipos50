<script>
	...
	function tagihanClick() {
		$headerContent.menuOpen = false;
		$headerContent.mode = "Tagihan";
		goto("/Tagihan");
	}
	...
	function updateSuplier(sup) {
		console.log(sup);
		$n_beli.suplierId = sup.id;
		$n_beli.suplierNama = sup.nama;
		$headerContent.suplierOpen = false;
	}

	function getHutang(suplier) {
		$n_beli.suplierId = suplier.id;
		$n_beli.suplierNama = suplier.nama;
		$headerContent.suplierOpen = false;

		//io.emit("getHutang", suplier.id);
...
<div class="w-full h-24 px-2 py-4">
	...
	>
		<div
			class=" h-12 bg-orange-500 rounded-xl rounded-b-none rounded-tr-none"
		>
			...
			<Dropdown
				...
				class="bg-orange-500 text-white  ml-4"
			>
				<DropdownItem
					on:click={() => {
						goto("/");
				...
				<DropdownDivider />
				<DropdownItem on:click={() => belanjaClick()}
					>Belanja</DropdownItem
				>
				<DropdownDivider />
				<DropdownItem on:click={() => tagihanClick()}
					>Tagihan</DropdownItem
				>
				<DropdownDivider />
				<DropdownItem on:click={() => antrianClick()}
					>Antrian</DropdownItem
				>
				<DropdownDivider />
				<DropdownItem on:click={() => dashboardClick()}
		...
		<div class="col-span-5 h-12 p-1 text-white bg-orange-500">
			{#if $headerContent.mode === "Dashboard"}
				...
				<div class="text-xs ml-4 font-bold">
					{getTanggal(Date.now())}
				</div>
			{:else if $headerContent.mode === "Kasir"}
				<div class="text-xl ml-4 font-bold">Penjualan</div>
				<div class="text-xs ml-4 font-bold">
					{getTanggal(Date.now())} ID: {$n_order.id}
				</div>
			{:else if $headerContent.mode === "Belanja"}
				<div class="text-xl ml-4 font-bold">Belanja</div>
				<div class="text-xs ml-4 font-bold">
					{getTanggal(Date.now())} ID: {$n_beli.id}
				</div>
			{:else if $headerContent.mode === "Tagihan"}
				<div class="text-xl ml-4 font-bold">Tagihan</div>
				<div class="text-xs ml-4 font-bold">
					{getTanggal(Date.now())}
				</div>
		...
		<div class="col-span-8 h-12 pb-1">
			...
			{:else if $headerContent.mode === "Kasir"}
				<div class="grid grid-cols-2 w-full h-11">
					<button
						class="w-full h-full border-r border-orange-500 p-0"
					>
						<div
							class="ml-1 text-left font-semibold"
							style="font-size:10px"
						>
							Pelanggan
						</div>
						<div class="font-bold text-xs">
							{$n_order.pelanggan.nama}
						</div>
					</button>
					<Dropdown
						padding="none"
					...
					>
						...
						{/each}
						<button
							on:click={() => pelangganBaruClick()}
							slot="footer"
							class="w-full text-center">Pelanggan Baru</button
						>
					</Dropdown>
					<button
						class="w-full h-full border-l border-orange-500 p-0"
					>
				...
				<div class="grid grid-cols-2 w-full h-11">
					...
					>
						{#each $dataSuplier as suplier}
							<DropdownItem
								on:click={() => updateSuplier(suplier)}
			...
			{:else if $headerContent.mode === "Antrian"}
				<div class="w-full h-11 grid grid-cols-3">
					<button
						...
						class="w-full border-t border-r border-white {$headerContent.antrianSelect ===
						1
							? 'bg-white text-black'
					...
					<button
						...
						class="w-full border-t border-white {$headerContent.antrianSelect ===
						3
							? 'bg-white text-black'
			...
			{:else if $headerContent.mode === "Setup"}
				<div class="w-full h-11 pb-1 grid grid-cols-4">
					<button
						...
						class="w-full border-t border-r border-white {$headerContent.setupSelect ===
						'menu'
							? 'bg-white text-black'
							: 'bg-orange-500 text-white'}"
						>Menu
					...
					>
						...
						{/if}
						<DropdownItem
							on:click={() => newMenuClick()}
							slot="footer"
							class="text-orange-500">Tambah Menu</DropdownItem
						>
					</Dropdown>
					<button
					...
					>
					<Dropdown
						bind:open={$headerContent.bahanOpen}
						class="h-96 ml-4 w-64 overflow-y-auto"
					>
						{#if $dataBahanStore.length > 0}
							{#each $dataBahanStore as bahan, index}
								<DropdownItem
									on:click={() => editBahanClick(bahan)}
									class="grid grid-cols-3 px-4"
								>
						...
						{/if}
						<DropdownItem
							on:click={() => newBahanClick()}
							slot="footer"
							class="text-orange-500">Tambah Bahan</DropdownItem
						>
					</Dropdown>
					<button
						on:click={() =>
					...
					<button
						on:click={() =>
							($headerContent.setupSelect = "suplier")}
						class="w-full border-t border-r border-white {$headerContent.setupSelect ===
						'suplier'
							? 'bg-white text-black'
					...
					>
						...
						{/if}
						<DropdownItem
							on:click={() => newSuplierClick()}
							slot="footer"
							class="text-orange-500">Tambah Suplier</DropdownItem
						>
					</Dropdown>
				</div>
			{:else if $headerContent.mode === "Tagihan"}
				<div class="grid grid-cols-2 w-full h-11">
					...
					>
						{#each $dataSuplier as suplier}
							<DropdownItem on:click={() => getHutang(suplier)}
								>{suplier.nama}</DropdownItem
							>
							<DropdownDivider />
						{/each}
					...
					<button  class="h-full w-full p-0">
						...
						<div class="font-bold text-right mr-2 mt-3">
							{rupiah($totalHutang)}
						</div></button
					>
				</div>
